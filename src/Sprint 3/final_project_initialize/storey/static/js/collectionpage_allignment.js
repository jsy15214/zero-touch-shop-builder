...
$(document).ready(function() {  // Runs when the document is ready
  ...
  function updateAllignmentChanges(data) {
    ...
    var filtering_content = document.getElementById("filtering_content");
    while (filtering_content.childElementCount > 0) {
      filtering_content.removeChild(filtering_content.lastChild);
    }
    if (data['allignment_filtering']) {
      $("#filtering_content").append($("#filtering_template").html());
    }
